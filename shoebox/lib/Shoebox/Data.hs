...
newtype TextLine = TX Text deriving (Show, Eq, Data, Typeable)

-- A TextLine is broken up into Morpheme's, which can be lexical elements, suffixed or prefixes
newtype MorphemeBreak = MB [Morpheme] deriving (Show, Eq, Data, Typeable)
data Morpheme = MorphemeLex Text | MorphemeSuffix Text | MorphemePrefix Text deriving (Show, Eq, Data, Typeable)

-- A GlossLine summarizes all Glosses, which are meanings attached to Morphemes
newtype GlossLine = GL [Choice] deriving (Show, Eq, Data, Typeable)
data Choice = MeaningChoice [Text] | AbbreviationChoice [Text] deriving (Show, Eq, Data, Typeable)

-- An interlinear block is a "categorized" textline, inluding its morphemes and glosses
data InterlinearBlock = ILB TextLine MorphemeBreak GlossLine deriving (Show, Eq, Data, Typeable)
